...
namespace ONS.AuthProvider.Common.Util.Http
{
    /// <summary>
    ///     Classe responsável por registrar dados de requisições, executada pelo client Http.
    ///     São registrados os dados de requisição e resposta da solicitação.
    /// </summary>
    public class LogHandler : DelegatingHandler
    {
        ...
        public LogHandler(HttpMessageHandler innerHandler, ILogger logger,
            KeyValuePair<string, string>[] replaces = null)
            : base(innerHandler)
        {
            _logger = logger;
            ...
            CancellationToken cancellationToken)
        {
            if (_logger.IsEnabled(LogLevel.Trace))
            {
                ...
                if (request.Content != null) sb.AppendLine(await request.Content.ReadAsStringAsync());
                sb.AppendLine();
            ...
            if (_logger.IsEnabled(LogLevel.Trace))
            {
                ...
                if (response.Content != null) sb.AppendLine(await response.Content.ReadAsStringAsync());
                sb.AppendLine();
